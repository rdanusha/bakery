...
class orddd_filter {
	...
	public static function orddd_display_delivery_date_admin_order_meta( $order ) {
        ...
        $field_date_label = orddd_common::orddd_get_delivery_date_field_label( $shipping_method, $product_category, $shipping_class, $location, $order_id ); 
	...
	public static function orddd_admin_order_preview_add_delivery_date( $data, $order ) {
		...
		if ( '' != $orddd_timestamp ) {
	        ...
	        $data[ 'payment_via' ] = $data[ 'payment_via' ] . '<br>' . '<strong>Delivery date</strong>' . $delivery_date;
