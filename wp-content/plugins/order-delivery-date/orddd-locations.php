...
class orddd_locations {
	...
	public function __construct() {
        ...
        add_action( 'woocommerce_checkout_process',     array( &$this, 'orddd_validate_pickup_locations' ) );
	}
     ...
     * @since 8.4
     */
    public function orddd_custom_delivery_settings( $shipping_settings, $shipping_settings_to_load, $j ) {
    	$shipping_method_str = '';
    	if( isset( $shipping_settings[ 'delivery_settings_based_on' ][ 0 ] ) && $shipping_settings[ 'delivery_settings_based_on' ][ 0 ] == 'orddd_locations' ) {
			...
			$shipping_settings_to_load[ 'unique_settings_key' ] = 'custom_settings_' . $j;
        }	
    ...
    public function orddd_validate_pickup_locations() {
    	global $current_user;
        $roles = array();
        if ( has_filter( 'orddd_disable_delivery_for_user_roles' ) ) {
            $roles = apply_filters( 'orddd_disable_delivery_for_user_roles', $roles );
