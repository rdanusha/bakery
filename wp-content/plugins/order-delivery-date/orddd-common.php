...
class orddd_common {
	...
	public static function orddd_get_order_estimated_text_block( $order_id ) {
        ...
        if( isset( $data[ 'orddd_estimated_shipping_timestamp' ] ) ) {
            ...
            if( '' != $orddd_estimated_shipping_timestamp ) {
            	...
            	$orddd_text_block = __( 'Your Estimated shipping date for the order is ', 'order-delivery-date' ) . "<b>" . $shipping_date . '</b>. </br>The delivery will take approximately <b>' . $orddd_min_between_days . "-" . $orddd_max_between_days . '</b> working days from the shipping days.' ;
            }
        }
	 ...
	 * @since 1.0
	 */
	public static function load_hidden_fields( $called_from = '' ) {
	    ...
	    if ( get_option( 'orddd_enable_time_slot' ) == 'on' ) {
	        ...
	        if( $last_slot_hrs != 0 ) {
		        ...
		        // This condition is added to check if the same day or next day is enabled or not. 
		        // If it is enable calculate the minimum available date on the current time. 
		        // Else, calculate for the Minimum order date calculated after Minimum delivery time. 
		        ...
		        // } else {
		        // 	$booking_date2       =   $current_date . " " . $last_slot;
		        // }
				$booking_date2       =   $min_date_array[ 'min_date' ] . " " . $last_slot;	
		        ...
		        if( '' !== $minimum_delivery_time && 0 != $minimum_delivery_time ) {
		    	...
		    	if ( version_compare( phpversion(), '5.3.0', '>' ) ) {
		            $calculated_difference        =   $calculated_min_date->diff( $date1 );
		        } else {
		            $calculated_difference        =   orddd_common::dateTimeDiff( $calculated_min_date, $date1 );
		        }
		        ...
		        if ( $difference->invert == 0 || $diff_hour_in_seconds < $calculated_minimum_delivery_time ) {
	    ...
	    if ( get_option( 'orddd_delivery_from_hours' ) != '' ) {
			if( $current_date == $min_date_array[ 'min_date' ] && 'on' == get_option( 'orddd_enable_next_day_delivery' ) && 'on' != get_option('orddd_enable_same_day_delivery' ) ) {
				...
				$var .= '<input type="hidden" name="orddd_min_hour" id="orddd_min_hour" value="' . $current_hour . '">';
	            $var .= '<input type="hidden" name="orddd_min_minute" id="orddd_min_minute" value="' . $current_minute . '">';
	            ...
	            $var .= '<input type="hidden" name="orddd_min_hour" id="orddd_min_hour" value="' . $min_date_array[ 'min_hour' ] . '">';
	            $var .= '<input type="hidden" name="orddd_min_minute" id="orddd_min_minute" value="' . $min_date_array[ 'min_minute' ] . '">';
		...
		return apply_filters( 'orddd_hidden_variables', $var );
	}
	...
	public static function orddd_get_zone_id( $order_id = "", $is_ajax = true ) {
		...
		if ( defined( 'WOOCOMMERCE_VERSION' ) && version_compare( WOOCOMMERCE_VERSION, "2.6.0", '>=' ) ) {
			...
			if ( false === $matching_zone_id ) {
				...
				$criteria[] = $wpdb->prepare( "OR ( location_type = 'postcode' AND location_code = %s )", $postcode );
	...
	public static function get_datepicker_options( $holidays_str, $lockout_str ) {
	    ...
	    if ( $time_settings_enable == 'on' ) {
	        ...
	        if ( get_option( 'orddd_delivery_to_hours' ) != '' ) {
	            ...
	            $options[] = "minuteMax:". get_option( 'orddd_delivery_to_mins' );
	        }
	...
	public static function orddd_untrash_order( $post_id ) {
	    ...
	    if ( 'shop_order' == $post_obj->post_type && ( !in_array( $post_obj->post_status, $status ) ) ) {
	        ...
	        orddd_common::orddd_restore_deliveries( $post_id, 'trashed', $post_obj->post_status );
	    }
	}
	...
	* @since 7.5
	*/
	public static function orddd_restore_deliveries( $order_id, $old_status, $new_status ) {
		if( ( $old_status == 'cancelled' && ( $new_status != 'cancelled' || $new_status != 'refunded' || $new_status != 'failed' ) ) 
			|| ( $old_status == 'refunded' && ( $new_status != 'cancelled' || $new_status != 'refunded' || $new_status != 'failed' ) ) 
	        || ( $old_status == 'trashed' && ( $new_status != 'cancelled' || $new_status != 'refunded' || $new_status != 'failed' ) ) 
	        || ( $old_status == 'failed' && ( $new_status != 'cancelled' || $new_status != 'refunded' || $new_status != 'trashed' ) ) ) {
            ...
            if( isset( $data[ '_orddd_timestamp' ][ 0 ] ) && $data[ '_orddd_timestamp' ][ 0 ] != '' ) {
                $delivery_date = date( "j-n-Y", $data[ '_orddd_timestamp' ][ 0 ] );
    ...
    public static function orddd_cancel_delivery( $order_id ) {
        ...
        $product_category = orddd_common::orddd_get_cart_product_categories( $order_id );
        $shipping_class = orddd_common::orddd_get_cart_shipping_classes( $order_id );
        //To check whether the below two lines is a proper fix as we need the aaray of categories and shipping_classes below. Also to check whether the above variables $product_category, $shipping_class are required
        $categories = orddd_common::orddd_get_cart_product_categories( $order_id );
        $shipping_classes = orddd_common::orddd_get_cart_shipping_classes( $order_id );
        
        $time_field_label = orddd_common::orddd_get_delivery_time_field_label( $shipping_method, $categories, $shipping_class, $location, $order_id ); 
        $timeslt = '';
	    ...
	    if( get_option( 'orddd_enable_shipping_based_delivery' ) == 'on' && is_array( $results ) && count( $results ) > 0 ) {
            ...
            if( is_array( $shipping_settings_to_check ) && count( $shipping_settings_to_check ) > 0 ) {
            	foreach ( $shipping_settings_to_check as $setting_key => $setting_value ) {
	            	if ( $setting_value[ 'date_lockout' ] != '' && $setting_value[ 'date_lockout' ] != '0' ) {
	                    if( $delivery_date_timestamp != '' ) {
	...
	public static function orddd_get_shipping_based_settings() {
	    ...
	    if( get_option( 'orddd_enable_shipping_based_delivery' ) == 'on' ) {
	        ...
	        foreach ( $results as $key => $value ) {    
	            ...
	            if( isset( $shipping_settings[ 'delivery_settings_based_on' ][ 0 ] ) && $shipping_settings[ 'delivery_settings_based_on' ][ 0 ] == 'shipping_methods' ) {
        			$shipping_method_js_str[ $i ][ 'unique_settings_key' ]             = 'custom_settings_' . $j;
                ...
                if( has_filter( 'orddd_custom_delivery_settings' ) ) {
	        		$shipping_method_js_str[ $i ] = apply_filters( 'orddd_custom_delivery_settings', $shipping_settings, $shipping_method_js_str[ $i ], $j );
	        	}
	            $i++;
	            $j++;
	        }
	...
	public static function orddd_get_shipping_hidden_variables( $shipping_settings, $timeslot_field_mandatory ) {
        ...
        if ( isset( $shipping_settings[ 'time_slots' ] ) && 
			...
			if( $last_slot_hrs != 0 ) {
                $last_slot           =   $last_slot_hrs . ':' . $last_slot_min;
                $advance_booking_hrs =   $shipping_settings[ 'minimum_delivery_time' ];

				$booking_date2       =   $min_date_array[ 'min_date' ] . " ". $last_slot;

				if( '' !== $advance_booking_hrs && 0 != $advance_booking_hrs ) {
					$calculated_date 	 =   $min_date_array[ 'min_date' ] . " " . $min_date_array[ 'min_hour' ] .":" . $min_date_array[ 'min_minute' ];
				}else {
					$calculated_date =   $current_date . " " . $current_hour .":" . $current_minute;;
				}
        ...
        $new_array[ "orddd_current_date_to_check" ] = $min_date_array[ 'current_date_to_check' ];
	...
	public static function orddd_get_all_future_orders() {
		global $wpdb;
		...
		$orddd_query = "SELECT ID, post_status FROM `" . $wpdb->prefix . "posts` WHERE post_type = 'shop_order' AND post_status NOT IN ( 'wc-on-hold', 'wc-cancelled', 'wc-failed', 'wc-refunded', 'trash' ) AND ID IN ( SELECT post_id FROM `" . $wpdb->prefix . "postmeta` WHERE ( meta_key = '_orddd_timestamp' AND meta_value >= '" . $current_time . "' ) )";
	 ...
	 * @since 4.1
	 */
	public static function orddd_get_total_product_quantities( $called_from = '' ) {
	    global $woocommerce;
	...
	*/ 
		...
		if( isset( $results ) ) {
    		...
    		if( '' != $zone_id ) {
	            if( false !== strpos( $shipping_method, 'usps' ) ) {
	                ...
	                $shipping_method = $zone_id . ":" . $shipping_method;
	            } else if( false !== strpos( $shipping_method, 'table_rate' ) ) {
	...
	public static function orddd_is_view_subscription_page() {
		...
		if( class_exists( 'WC_Subscriptions' ) && wcs_is_view_subscription_page() ) {
