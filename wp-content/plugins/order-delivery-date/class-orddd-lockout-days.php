...
class ORDDD_Lockout_Days {
	...
	public static function orddd_get_booked_timeslot_days() {
		...
		foreach ( $lockout_arr as $date => $timeslot ) {
			...
			if ( is_array( $specific_dates ) && count( $specific_dates ) > 0 && get_option( 'orddd_enable_specific_delivery_dates' ) == 'on' ) {
				$specific_date = date( 'n-j-Y', strtotime( $date_lockout ) );
				if ( isset( $specific_dates[ $specific_date ] ) && is_array( $lockout_arr[ $date ] ) && count( $lockout_arr[ $date ] ) >= count( $specific_dates[ $specific_date ] ) ) {
					array_push( $booked_dates, $date_lockout );
				}
			}

			if ( is_array( $delivery_days ) && count( $delivery_days ) > 0 ) {
				$weekday = date( 'w', strtotime( $date_lockout ) );
				if ( isset( $delivery_days[ 'orddd_weekday_' . $weekday ] ) && is_array( $lockout_arr[ $date ] ) && count( $lockout_arr[ $date ] ) >= count( $delivery_days[ 'orddd_weekday_' . $weekday ] ) ) {
					array_push( $booked_dates, $date_lockout );
				}
			}
		}
	...
	public static function orddd_get_custom_booked_timeslots( $shipping_settings ) {
		...
		// For time slot lockout the date format saved in the database is j-n-Y. And the date format we add in the booked days array is n-j-Y.
		foreach ( $lockout_arr as $date => $time ) {
			$lockout_date_split = explode( '-', $date );
			$date_lockout       = $lockout_date_split[1] . '-' . $lockout_date_split[0] . '-' . $lockout_date_split[2]; // n-j-Y

			$specific_date = date( 'n-j-Y', strtotime( $date ) );
			if ( isset( $shipping_settings['delivery_type']['specific_dates'] ) && $shipping_settings['delivery_type']['specific_dates'] == 'on' && isset( $specific_dates[ $specific_date ] ) && is_array( $lockout_arr[ $date ] ) && count( $lockout_arr[ $date ] ) >= count( $specific_dates[ $specific_date ] ) ) {
				array_push( $booked_dates, $date_lockout );
			}

			$weekday = date( 'w', strtotime( $date ) );

			if ( isset( $delivery_days[ 'orddd_weekday_' . $weekday . '_custom_setting' ] ) && is_array( $lockout_arr[ $date ] ) && count( $lockout_arr[ $date ] ) >= count( $delivery_days[ 'orddd_weekday_' . $weekday . '_custom_setting' ] ) ) {
				array_push( $booked_dates, $date_lockout );
			}
		}
