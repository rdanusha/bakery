...
include_once( 'includes/class-orddd-date-settings.php' );
include_once( 'includes/class-orddd-shipping-days-settings.php' );
include_once( 'includes/class-orddd-time-settings.php' );
include_once( 'includes/class-orddd-holidays-settings.php' );
include_once( 'includes/class-orddd-appearance-settings.php' );
include_once( 'includes/orddd-delivery-days-settings.php' );
include_once( 'includes/class-orddd-time-slot-settings.php' );
include_once( 'includes/class-orddd-additional-settings.php' );
include_once( 'includes/class-orddd-shipping-based-settings.php' );
include_once( 'includes/class-orddd-calendar-sync-settings.php' );
    ...
    */ 
    ...
    public  function orddd_time_slot_settings() {
        ...
        add_settings_field(
            ...
            array( 'orddd_time_slot_settings', 'orddd_time_to_hours_callback' ),
            'orddd_time_slot_page',
            'orddd_add_time_slot_section',
    ...
    public function orddd_shipping_based_settings_callback() {
        ...
        add_settings_field(
            ...
            'orddd_shipping_based_page',
            'orddd_shipping_based_date_settings_section',
            array ( __( 'Minimum number of hours required to prepare for delivery. 
            <br><em> Note: This setting will be applied with Same day & Next day cutoff. Please refer <a href="https://www.tychesoftwares.com/how-does-minimum-delivery-time-work-with-same-day-next-day-cutoff-settings/" target="_blank">this post</a> to know more.</em>', 'order-delivery-date' ) ) 
        );
        ...
        $row_id = '';
        if ( ( isset( $_GET[ 'action' ] ) && sanitize_text_field( $_GET[ 'action' ] ) == 'shipping_based' ) && ( isset( $_GET[ 'mode' ] ) && sanitize_text_field( $_GET[ 'mode' ] ) == 'edit' ) ) {
	...
	public function order_delivery_date_settings() {
		...
		$check_prev = array();
		$action = $active_general_settings = $calendar_sync_settings = $active_shipping_based = $active_advance_settings = "";
		if ( isset( $_GET[ 'action' ] ) ) {
		...
		if ( $action == 'advance_settings' ) {
		    $active_advance_settings = "nav-tab-active";
		}
		
		if ( $action == 'shipping_based' ) {
		    $active_shipping_based = "nav-tab-active";
		}
		
		if( $action == 'calendar_sync_settings' ) {
        ...
        <div class="wrap woocommerce">
            <nav class="nav-tab-wrapper woo-nav-tab-wrapper" id="orddd_settings_tabs">
                <a href="admin.php?page=order_delivery_date&action=general_settings" class="nav-tab <?php echo $active_general_settings; ?>"><?php _e( 'General Settings', 'order-delivery-date' );?> </a>
                <?php 
                if ( get_option( 'orddd_enable_day_wise_settings' ) == 'on' ) {
                    ?>
                    <a href="admin.php?page=order_delivery_date&action=advance_settings" class="nav-tab <?php echo $active_advance_settings; ?>"> <?php _e( 'Weekday Settings', 'order-delivery-date' ); ?> </a>
                <?php } ?>
                <a href="admin.php?page=order_delivery_date&action=shipping_based" class="nav-tab <?php echo $active_shipping_based; ?>"> <?php _e( 'Custom Delivery Settings', 'order-delivery-date' ); ?> </a>
    			<a href="admin.php?page=order_delivery_date&action=calendar_sync_settings" class="nav-tab <?php echo $calendar_sync_settings; ?>"> <?php _e( 'Google Calendar Sync', 'order-delivery-date' ); ?> 
                </a>
                <?php
		...
		if( $action == 'general_settings' ) {
		    ...
		    if ( $section == 'time_slot' ) {
                ...
                <?php _e( 'Block Time Slots', 'order-delivery-date' );
		...
		} else if( $action == 'advance_settings' ) {

            include_once( 'includes/views/html-advance-settings.php' );

		} else if ( $action == 'shipping_based' && $mode != 'add_settings' && $mode != 'edit' ) {
            print( '<div id="content">
                <form method="post" action="options.php">');
            ...
            if( $number_of_shipping_settings === 0 && 
                ...
                <div id = "orddd_shipping_based_settings_list">
                    <form id="custom-delivery-settings" method="get" >
                        <input type="hidden" name="page" value="order_delivery_date" />
        ...
        } else if ( $action == 'calendar_sync_settings' ) {
            print( '<div id="content">
                <form method="post" action="options.php">');
	...
	public function orddd_delete_settings() {
	    if ( ( isset( $_GET[ 'page' ] ) && sanitize_text_field( $_GET[ 'page' ] ) == 'order_delivery_date' ) && 
            ( isset( $_GET[ 'tab' ] ) && sanitize_text_field( $_GET[ 'tab' ] ) == 'shipping_based' ) ) {
	        if( ( isset( $_GET[ 'action' ] ) && sanitize_text_field( $_GET[ 'action' ] ) == 'orddd_delete' ) || 
                ( isset( $_GET[ 'action2' ] ) && sanitize_text_field( $_GET[ 'action2' ] ) == 'orddd_delete') ) {
    	        $custom_delivery_setting = array();
    	        if( isset( $_GET[ 'custom_delivery_setting' ] ) ) {
    	            $custom_delivery_setting = $_GET[ 'custom_delivery_setting' ];
    	        }
	    ...
	    if ( ( isset( $_GET[ 'page' ] ) && sanitize_text_field( $_GET[ 'page' ] ) == 'order_delivery_date' ) && 
            ( isset( $_GET[ 'tab' ] ) && sanitize_text_field( $_GET[ 'tab' ] ) == 'general_settings' ) && 
            ( isset( $_GET[ 'section' ] ) && sanitize_text_field( $_GET[ 'section' ] ) == 'delivery_dates' ) ) {
	        if( ( isset( $_GET[ 'action' ] ) && sanitize_text_field( $_GET[ 'action' ] ) == 'orddd_delete' ) || 
                ( isset( $_GET[ 'action2' ] ) && sanitize_text_field( $_GET[ 'action2' ] ) == 'orddd_delete') ) {
    	        $delivery_date = array();
    	        ...
    	        foreach( $delivery_date as $d_key => $d_value ) {
    	            ...
    	            foreach ( $time_slot as $key => $v ) {
    	                ...
    	                if ( gettype(json_decode( $v->dd ) ) == 'array' && count( json_decode( $v->dd ) ) > 0 ) {
    	                    ...
    	                    if ( trim( $new_dd_str )!= "" ) {
    	                        $new_dd_str .= ']';
    	                        $timeslot_new_arr[] = array( 'tv' => $v->tv, 'dd' => $new_dd_str, 'lockout' => $v->lockout, 'additional_charges' => $v->additional_charges, 'additional_charges_label' => $v->additional_charges_label, 'fh' => $v->fh, 'fm' => $v->fm, 'th' => $v->th, 'tm' => $v->tm );
    	                    }
    	                } else {
	    ...
	    if ( ( isset( $_GET[ 'page' ] ) && sanitize_text_field( $_GET[ 'page' ] ) == 'order_delivery_date' ) && 
            ( isset( $_GET[ 'tab' ] ) && sanitize_text_field( $_GET[ 'tab' ] ) == 'general_settings' ) && 
            ( isset( $_GET[ 'section' ] ) && sanitize_text_field( $_GET[ 'section' ] ) == 'holidays' ) ) {
	        if( ( isset( $_GET[ 'action' ] ) && sanitize_text_field( $_GET[ 'action' ] ) == 'orddd_delete' ) || 
                ( isset( $_GET[ 'action2' ] ) && sanitize_text_field( $_GET[ 'action2' ] ) == 'orddd_delete' ) ) {
	    ...
	    if ( ( isset( $_GET[ 'page' ] ) && sanitize_text_field( $_GET[ 'page' ] ) == 'order_delivery_date' ) && ( isset( $_GET[ 'tab' ] ) && sanitize_text_field( $_GET[ 'tab' ] ) == 'general_settings' ) && ( isset( $_GET[ 'section' ] ) && sanitize_text_field( $_GET[ 'section' ] ) == 'time_slot' ) ) {
	        if( ( isset( $_GET[ 'action' ] ) && sanitize_text_field( $_GET[ 'action' ] ) == 'orddd_delete' ) || ( isset( $_GET[ 'action2' ] ) && sanitize_text_field( $_GET[ 'action2' ] ) == 'orddd_delete') ) {
    	        $time_slot_to_delete = array();
    	        if( isset( $_GET[ 'time_slot' ] ) ) {
    	            $time_slot_to_delete = $_GET[ 'time_slot' ];
    	        }
	    ...
	    if ( ( isset( $_GET[ 'page' ] ) && sanitize_text_field( $_GET[ 'page' ] ) == 'order_delivery_date' ) && ( isset( $_GET[ 'tab' ] ) && sanitize_text_field( $_GET[ 'tab' ] ) == 'general_settings' ) && ( isset( $_GET[ 'section' ] ) && sanitize_text_field( $_GET[ 'section' ] ) == 'block_time_slot_settings' )  ) {
	        if( ( isset( $_GET[ 'action' ] ) && sanitize_text_field( $_GET[ 'action' ] ) == 'orddd_delete' ) || ( isset( $_GET[ 'action2' ] ) && sanitize_text_field( $_GET[ 'action2' ] ) == 'orddd_delete') ) {
    	        $block_time_slot_to_delete = array();
    	        if( isset( $_GET[ 'block_time_slot' ] ) ) {
    	            $block_time_slot_to_delete = $_GET[ 'block_time_slot' ];
    	        }
    	        ...
    	        foreach( $block_time_slot_to_delete as $t_key => $t_value ) {
    	            ...
    	            foreach( $disable_time_slots as $disable_key => $disable_v ) {
    	                ...
    	                if ( ( isset( $time_Slot ) && in_array( $time_Slot, $time_slots ) ) && ( isset( $date_to_check ) && $date_to_check == $disable_v->dd ) ) {
    	                    ...
    	                    if ( trim( $new_ts_str ) != "" ) {
    	                        $new_ts_str .= ']';
    	                        $timeslot_disable_new_arr[] = array( 'dtv' => $disable_v->dtv, 'dd' => $disable_v->dd, 'ts' => $new_ts_str );
    	                    }
    	                } else {
    	                    $timeslot_disable_new_arr[] = array(
    	                        'dtv'     => $disable_v->dtv,
	...
	public function orddd_save_category_fields( $term_id, $tt_id, $taxonomy ) {	
        if ( isset( $_POST[ 'orddd_delivery_date_for_product_category' ] ) ) {
            ...
            if( ( isset( $_POST[ 'newproduct_cat' ] ) && $_POST[ 'newproduct_cat' ] != '' ) || ( isset( $_POST[ 'action' ] ) && $_POST[ 'action' ] == 'wpml_save_job_ajax' ) ) {
                update_term_meta( $term_id, 'orddd_delivery_date_for_product_category', 'on' );
            } else {
                update_term_meta( $term_id, 'orddd_delivery_date_for_product_category', '' );
            }
        }
        ...
        foreach ( $changes as $row_id => $data ) {
            ...
            if( isset( $row_id_arr[ 0 ] ) && 'new' == $row_id_arr[ 0 ] ) {
                foreach( $data as $data_key => $data_value ) {
                    ...
                    } else if( 'delivery_charges_label' == $data_key ) {
                ...
                foreach( $data as $data_key => $data_value ) {
                    ...
                    } else if( 'additional_charges' == $data_key ) {
                        $previous_advance_settings = $advance_settings[ $row_id ];
