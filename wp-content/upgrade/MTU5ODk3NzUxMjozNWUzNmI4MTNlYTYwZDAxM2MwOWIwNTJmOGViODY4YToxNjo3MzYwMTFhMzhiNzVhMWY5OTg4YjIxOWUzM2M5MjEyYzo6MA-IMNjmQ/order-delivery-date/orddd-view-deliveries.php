							...
							function addURL( element ) {
								jQuery( element ).attr( 'href', function() {
									...
									if ( orddd_this_href.includes( "orddd_data.print" ) ) {
										orddd_this_href = "<?php echo get_admin_url(); ?>/admin.php?page=orddd_view_orders&download=orddd_data.print";
									} else {
										orddd_this_href = "<?php echo get_admin_url(); ?>/admin.php?page=orddd_view_orders&download=orddd_data.csv";
									}
			...
			if ( 'directly' === get_option( 'orddd_calendar_sync_integration_mode' ) &&
				...
				jQuery( document ).ready( function(){ 
					jQuery( "#orddd_admin_add_to_calendar_delivery" ).on( 'click', function() {
						var orders_to_export = "
						<?php
						if ( is_array( $total_orders_to_export ) ) {
							echo count( $total_orders_to_export ); }
						?>
						";
							...
							if ( is_array( $variation_data ) && count( $variation_data ) > 0 ) {
								...
								foreach ( $meta_data as $mkey => $mvalue ) {
									...
									if ( isset( $meta_key['key'] ) &&
										...
										$variable_product_data .= $attribute_name . ': ' . urldecode( $meta_key['value'] ) . ', ';
									}
									$j++;
								}
