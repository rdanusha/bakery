<?php
/**
 * Order Delivery Date global variables
 *
 * All the Global arrays are declared.
 *
 * @author Tyche Softwares
 * @package Order-Delivery-Date-Pro-for-WooCommerce/Configuration
 * @since 
 */
...
global $orddd_calendar_themes, $orddd_weekdays, $orddd_shipping_days, $orddd_calendar_languages, $orddd_date_formats, $orddd_number_of_months, $orddd_time_formats, $orddd_days, $orddd_languages, $orddd_languages_locale;

$orddd_languages_locale = array(
    ...
    'Dutch Belgian'       => array( 'nl_BE.utf8', 'nld', 'nl-BE' ),
    ...
    'Norwegian'           => array( 'nn_NO.utf8', 'nob' ),
...
$orddd_number_of_months = array( 
    1 => 1, 
    2 => 2 );

$orddd_time_formats = array( 
    1 => "12 hour",
	2 => "24 hour" );

$orddd_calendar_themes = array( 
...
define( 'ORDDD_DELIVERY_DATE_FIELD_LABEL',       'Delivery Date' );
define( 'ORDDD_DELIVERY_DATE_FIELD_PLACEHOLDER', 'Choose a Date' );
define( 'ORDDD_DELIVERY_DATE_FIELD_NOTE',        'We will try our best to deliver your order on the specified date.' );
define( 'ORDDD_DELIVERY_DATE_FORMAT',            'd MM, yy' );
define( 'ORDDD_LOCKOUT_DATE_FORMAT',             'n-j-Y' );
define( 'ORDDD_HOLIDAY_DATE_FORMAT',             'n-j-Y' );
define( 'ORDDD_CALENDAR_THEME',                  'smoothness' );
define( 'ORDDD_CALENDAR_THEME_NAME',             'Smoothness' );
define( 'ORDDD_DELIVERY_TIMESLOT_FIELD_LABEL',   'Time Slot' );
define( 'ORDDD_PICKUP_DATE_FIELD_LABEL',         'Pickup Location' );

$plugin_is_activated = 'no';
define( 'ORDDD_SHOPPING_CART_HOOK_AMAZON', 'woocommerce_checkout_after_customer_details' );
...
if ( 'billing_section' == get_option( 'orddd_delivery_date_fields_on_checkout_page' ) ) {
	...
	define( 'ORDDD_SHOPPING_CART_HOOK', 'woocommerce_before_order_notes' );
} else if ( 'after_order_notes' == get_option( 'orddd_delivery_date_fields_on_checkout_page' ) ) {
	define( 'ORDDD_SHOPPING_CART_HOOK', 'woocommerce_after_order_notes' );
} else if ( 'after_your_order_table' == get_option( 'orddd_delivery_date_fields_on_checkout_page' ) ) {
    define( 'ORDDD_SHOPPING_CART_HOOK', 'woocommerce_review_order_before_payment' );
} else if ( 'custom' == get_option( 'orddd_delivery_date_fields_on_checkout_page' ) ) {
    $orddd_custom_hook_for_fields_placement = get_option( 'orddd_custom_hook_for_fields_placement' );
    define( 'ORDDD_SHOPPING_CART_HOOK', $orddd_custom_hook_for_fields_placement );
} else {
    define( 'ORDDD_SHOPPING_CART_HOOK', 'woocommerce_after_checkout_billing_form' );
}

define( 'ORDDD_TEMPLATE_PATH', untrailingslashit( plugin_dir_path( __FILE__ ) ) . '/templates/' );
?>